...
import { deleteContent, searchContents, updateContent, type SearchContentsQuery } from "../api/contents.endpoints";
...
import { addContentToCollection, getUserCollections } from "../../collections/api/collections.endpoints";
...
export default function ContentsSearchPage() {
  ...
  const handleCollectionAdd = async (collectionId: string) => {
    if (!selectedContent) return;
      await addContentToCollection({ contentId: selectedContent.id, userCollectionId: collectionId, userId: userId! });
      setModalType(null);
      searchContents(search).then(setSearchResults);
  };
